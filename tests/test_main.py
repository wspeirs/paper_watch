...
class TestMain(unittest.TestCase):
    ...
    @patch('src.paper_watch.main.SSRNClient')
    ...
    def test_main_flow(self, mock_extract, mock_download, mock_email, mock_gemini, mock_ssrn, mock_arxiv):
        # Setup mocks
        ...
        mock_ssrn_inst = mock_ssrn.return_value
        mock_ssrn_inst.fetch_papers.return_value = []
        
        mock_gemini_inst = mock_gemini.return_value
        mock_gemini_inst.screen_abstract.return_value = MagicMock(keep=True, reasoning="Keep it")
        mock_gemini_inst.analyze_full_paper.return_value = MagicMock(
            summary="Sum", methodology="Meth", data="Data", results="Res", 
            relevance_reconfirmed=True, reasoning="Still good"
        )
